...
export class ActivityHistoryClient {
  ...
  async add(params: {
    ...
    try {
      ...
      clientLogger.debug('[ActivityHistory] Failed to add activity:', error);
      return null;
    }
  }
  ...
  async get(filter: ActivityFilter = {}): Promise<ActivityPage | null> {
    try {
      ...
      clientLogger.debug('[ActivityHistory] Failed to get activities:', error);
      return null;
    }
  }
  ...
  async getRecent(limit: number = 10): Promise<Activity[]> {
    ...
    try {
      ...
      clientLogger.warn('[ActivityHistory] Activity history not yet initialized or empty:', error);
      return [];
    }
  }
  ...
  async countByType(): Promise<Record<string, number>> {
    try {
      ...
      clientLogger.warn('[ActivityHistory] Failed to count activities:', error);
      return {};
    }
  }
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      clientLogger.warn('[ActivityHistory] Failed to delete activity:', error);
      return false;
    }
  }
  ...
  async clear(): Promise<boolean> {
    try {
      ...
      clientLogger.warn('[ActivityHistory] Failed to clear activities:', error);
      return false;
    }
  }
