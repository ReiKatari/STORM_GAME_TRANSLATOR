...
export default function GameDetailPage() {
  ...
  const handleInstallUnityPatch = async (engine: string = 'google') => {
    if (!game) return;
  ...
  const handleInstallUnrealPatch = async (engine: string = 'google') => {
    if (!game?.installPath) return;
    ...
    try {
      const result = await invoke<string>('install_unreal_patch', {
        gamePath: game.installPath,
        config: {
          target_language: targetLang || 'ru',
          source_language: 'en',
          translation_service: engine,
          cache_enabled: true,
          auto_translate: true
        }
      });
