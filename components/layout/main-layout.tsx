...
import { BackgroundJobsIndicator, BackgroundJobsWidget } from '@/components/translator/background-jobs-widget';
...
import { DocumentationModal } from '@/components/tutorial/documentation-modal';
...
const getNavGroups = (t: (key: string) => string) => [
  // ── CORE ────────────────────────────────────────────────────────
  {
    label: t('nav.core'),
    items: [
      { name: t('nav.fullProcess') || '⚡ Полный процесс', href: '/', icon: Zap },
  ...
  // ── PATCHER ─────────────────────────────────────────────────────
  {
    label: t('nav.patcher') || 'Patcher',
    icon: Wand2,
    collapsible: true,
    items: [
      { 
        name: t('nav.patcherEngine') || 'Patcher Engine', 
        href: '/unity-patcher', 
        icon: Wand2,
        subItems: [
      ...
      { name: t('nav.videoExtractor') || 'Video Extractor', href: '/video-extractor', icon: Film },
      { name: t('nav.lipSync') || 'Lip Sync', href: '/lip-sync', icon: Smile },
  ...
  return (
    <TutorialProvider>
      ...
      <div className="flex h-full bg-background">
            ...
            {sidebarOpen && (
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="relative flex items-center justify-center shrink-0">
                  <img 
                    src="/logohires.png" 
                    alt="STORM GAME TRANSLATOR" 
                    className="h-[36px] w-auto animate-logo-glow"
                    style={{
                ...
                <div className="flex flex-col min-w-0">
                  <span 
                    className="text-base font-black tracking-wider bg-clip-text text-transparent truncate"
                    style={{
                      backgroundImage: 'linear-gradient(90deg, #60a5fa, #38bdf8, #818cf8, #60a5fa)',
                  ...
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.18em] -mt-0.5 truncate">TRANSLATOR</span>
                </div>
              </div>
            )}
                      ...
                      <div className="space-y-0.5">
                        {group.items.map((item: NavItem) => {
                          ...
                          const isActive = item.href === '/' 
                            ? (pathname === '/' || pathname === '/full-process') 
                            : pathname === item.href;
                          ...
                          return (
                            <Link key={item.href} href={item.href}>
                              <Button
                                variant="ghost"
                                data-tutorial={`nav-${item.href === '/' ? 'full-process' : item.href.replace('/', '')}`}
                                className={cn(
          ...
          <div className="shrink-0 px-3 py-2 flex items-center justify-center bg-slate-900/30 border-t border-slate-800/50">
            {sidebarOpen ? (
              <button 
                ...
                className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-800/50 transition-colors cursor-pointer group text-left"
              >
                <div className="p-1 rounded bg-indigo-500/20 text-indigo-400 group-hover:bg-indigo-500/30 transition-colors">
                  <BookOpen className="h-3.5 w-3.5" />
                </div>
                <span className="text-2xs font-bold text-slate-400 group-hover:text-slate-200 transition-colors uppercase tracking-wider">{t('nav.tutorialAndGuide')}</span>
              </button>
            ) : (
              <button 
                onClick={() => setDocOpen(true)}
                className="p-2 rounded-lg hover:bg-slate-800/50 transition-colors cursor-pointer text-indigo-400"
                title={t('nav.tutorialAndGuide')}
              >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center bg-slate-900/40 border border-slate-800/60 rounded-lg p-1 mr-2 hidden md:flex">
                <button 
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 text-2xs font-medium text-slate-300 hover:text-cyan-400 hover:bg-slate-800/50 rounded-md transition-all"
              ...
              {/* Кнопка Благодарностей в Header */}
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setCreditsOpen(true)}
                className="h-8 px-2.5 text-xs text-pink-300 hover:text-pink-200 bg-pink-950/20 hover:bg-pink-900/30 border border-pink-500/20 rounded-lg gap-1.5 font-medium transition-all"
                title="Благодарности, Авторы и Инструменты"
              >
                <Heart className="w-3.5 h-3.5 fill-pink-500/30 text-pink-400" />
                <span className="hidden md:inline">Благодарности</span>
              </Button>
        ...
        {/* Credits Modal (Благодарности и Авторы) */}
        <CreditsModal open={creditsOpen} onOpenChange={setCreditsOpen} />

        {/* Documentation & Manual Modal (Руководство и Инструкция) */}
                ...
                onClick={async () => {
                  try {
                    ...
                    if (isTauriEnv) {
                      ...
                      // Режим браузера: сбрасываем сессию и закрываем модалку
                    ...
                    // Gracefully handle browser fallback without error spam
                    setExitDialogOpen(false);
                  }
                }}
              >
