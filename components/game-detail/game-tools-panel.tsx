...
interface GameToolsPanelProps {
  ...
  dlcGames: unknown[];
  onScan: () => void;
  onInstallUnityPatch: (engine?: string) => void;
  onInstallUnrealPatch: (engine?: string) => void;
          ...
          <div className="space-y-2">
            {game.engine === 'Unity' && (
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-sky-500/[0.06] border border-sky-500/15">
                ...
                <div className="flex items-center gap-2">
                  <select
                    className="bg-[#1b2838] border border-[#2a475e] text-[#c6d4df] rounded-lg text-xs px-2 py-1 outline-none"
                  ...
                  <Button size="xs" className="text-micro font-bold bg-sky-500/15 hover:bg-sky-500/25 text-sky-300 rounded-lg border border-sky-500/20 px-2.5" onClick={() => onInstallUnityPatch(translationEngine)} disabled={isInstallingPatch}>
            ...
            {game.engine === 'Unreal Engine' && (
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-violet-500/[0.06] border border-violet-500/15">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button size="xs" className="text-micro font-bold bg-violet-500/15 hover:bg-violet-500/25 text-violet-300 rounded-lg border border-violet-500/20 px-2.5" onClick={() => onInstallUnrealPatch(translationEngine)} disabled={isInstallingPatch}>
                    {isInstallingPatch ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3 mr-1" />} Installa
                  </Button>
                </div>
              </div>
