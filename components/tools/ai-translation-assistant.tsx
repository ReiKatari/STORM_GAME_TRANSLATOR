...
const textTypes = [
  { value: 'dialogue', label: 'Диалог', icon: '💬' },
  { value: 'ui', label: 'Интерфейс', icon: '🖥️' },
  { value: 'item', label: 'Предмет', icon: '🎒' },
  { value: 'quest', label: 'Квест', icon: '📜' },
  { value: 'lore', label: 'Лор / Книги', icon: '📚' },
];

const tones = [
  { value: 'serious', label: 'Серьезный' },
  { value: 'comedic', label: 'Комедийный' },
...
export function AITranslationAssistant() {
  ...
  const handleTranslate = async () => {
    if (!inputText.trim()) {
      toast.error(t('aiTranslation.enterText') || 'Введите текст для перевода');
      return;
    }

    const provider = providers.find(p => p.id === selectedProvider);
    if (!provider?.isAvailable && provider?.type === 'local') {
      toast.error(`${provider.name} недоступен. Запустите его перед переводом.`);
      return;
    }
    ...
    try {
      ...
      toast.success(`Переведено за ${result.processingTime}мс`);
    } catch (error: unknown) {
      toast.error(error instanceof Error ? error.message : 'Ошибка перевода');
    } finally {
  ...
  const copyToClipboard = (text: string) => {
    ...
    toast.success(t('common.copied') || 'Скопировано в буфер обмена');
  };
  ...
  const addGlossaryTerm = () => {
    if (glossaryTerm && glossaryTranslation) {
      ...
      toast.success('Термин добавлен в глоссарий');
    }
  };
      ...
      {!localProviderAvailable && !isCheckingProviders && (
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-yellow-500/10 border border-yellow-500/20 text-xs">
          ...
          <span className="text-yellow-300">
            Ollama не обнаружена. Установите с <a href="https://ollama.ai" target="_blank" className="underline font-semibold">ollama.ai</a> → <code className="bg-slate-800 px-1 rounded text-2xs">ollama run translategemma</code>
          </span>
        </div>
      )}
        ...
        <div className="lg:col-span-8 space-y-3">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Input
              ...
              placeholder={t('aiTranslation.characterPlaceholder') || 'Имя персонажа'}
              className="h-9 w-40"
            />
          </div>
          ...
          <Textarea
            ...
            placeholder={t('aiTranslation.enterText') || 'Введите текст для перевода...'}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Switch
                ...
                className="scale-90"
              />
              <Label className="text-xs text-slate-400">{t('aiTranslation.alternative') || 'Альтернативы'}</Label>
            </div>
            <Button 
            ...
            >
              {isTranslating ? (
                <>
                  ...
                  {t('common.translating') || 'Перевод...'}
                </>
              ) : (
                <>
                  <Sparkles className="h-3.5 w-3.5 mr-1.5" />
                  {t('common.translate') || 'Перевести'}
                </>
              )}
            </Button>
          </div>
        ...
        <div className="lg:col-span-4 space-y-2">
            ...
            <Card className="border-slate-700/50">
              ...
              <CollapsibleContent>
                <CardContent className="space-y-4 pt-0">
                  <div className="space-y-2">
                    <Label>{t('aiTranslation.gameTitle') || 'Название игры'}</Label>
                    <Input
                      value={gameContext.gameTitle}
                      onChange={(e) => setGameContext(prev => ({ ...prev, gameTitle: e.target.value }))}
                      placeholder={t('aiTranslation.gameTitlePlaceholder') || 'например, Ведьмак 3'}
                    />
                  </div>

                  <div className="space-y-2">
                    <Label>{t('aiTranslation.genre') || 'Жанр'}</Label>
                    <Select 
                  ...
                  <div className="space-y-2">
                    <Label>{t('aiTranslation.tone') || 'Тон диалогов'}</Label>
                    <Select 
                  ...
                  <div className="space-y-2">
                    <Label>{t('aiTranslation.setting') || 'Сеттинг'}</Label>
                    <Input
                      value={gameContext.setting}
                      onChange={(e) => setGameContext(prev => ({ ...prev, setting: e.target.value }))}
                      placeholder={t('aiTranslation.settingPlaceholder') || 'например, темное фэнтези'}
                    />
                  </div>
                </CardContent>
              </CollapsibleContent>
          ...
          <Card className="border-slate-700/50">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-700/30">
              ...
              <span className="text-sm font-medium">{t('aiTranslation.glossary') || 'Глоссарий'}</span>
            </div>
            <CardContent className="space-y-2 p-3">
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder={t('aiTranslation.termPlaceholder') || 'Термин'}
                  className="w-28 min-w-0"
                />
                <Input
                  value={glossaryTranslation}
                  onChange={(e) => setGlossaryTranslation(e.target.value)}
                  placeholder={t('aiTranslation.translationPlaceholder') || 'Перевод'}
                  className="w-28 min-w-0"
                />
