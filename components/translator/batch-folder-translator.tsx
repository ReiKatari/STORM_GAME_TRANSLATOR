  ...
  const handleSelectFolder = async () => {
    try {
      ...
      if (selected) {
        ...
        // Listen for real-time file found events
        const unlisten = await listen("batch-file-found", (_event) => {
            ...
            >
              {isScanning ? (
                <>
                  ...
                  {scannedFilesCount > 0 && (
                    <p className="text-xs text-primary font-medium">Trovati {scannedFilesCount} file translatable...</p>
                  )}
                </>
              ) : (
                <>
