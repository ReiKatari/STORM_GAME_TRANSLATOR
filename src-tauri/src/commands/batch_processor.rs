...
pub fn scan_folder_for_translation(
    ...
    // Raccogli tutti i path candidati in modo sincrono
    ...
    // Processa in parallelo per calcolare gli entry (che richiede lettura da disco)
    let files: Vec<BatchFile> = candidate_paths.par_iter().filter_map(|entry_path| {
        let extension = entry_path
            .extension()
            .map(|e| e.to_string_lossy().to_lowercase())
            .unwrap_or_default();
        ...
        // Emetti un evento per aggiornare la UI in tempo reale
        let _ = app.emit("batch-file-found", &batch_file);
