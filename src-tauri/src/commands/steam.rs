...
/// Trova tutti i giochi installati scansionando i file .acf (in parallelo)
fn find_installed_games(library_folders: &[SteamLibraryFolder]) -> Result<Vec<LocalGameInfo>, String> {
    let mut acf_paths = Vec::new();
    ...
    let games: Vec<LocalGameInfo> = acf_paths.par_iter()
        .filter_map(|path| parse_acf_file(path).ok())
        .collect();
