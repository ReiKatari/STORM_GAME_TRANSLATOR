...
fn main() {
    ...
    let app_data_dir = if cfg!(debug_assertions) {
        if std::path::Path::new("gamestringer_data").exists() {
            std::path::PathBuf::from("gamestringer_data")
        } else if std::path::Path::new("../gamestringer_data").exists() {
            std::path::PathBuf::from("../gamestringer_data")
        } else {
            std::path::PathBuf::from("gamestringer_data")
        }
    ...
    let settings_manager = ProfileSettingsManager::new(app_data_dir.clone()).expect("Failed to initialize settings manager");
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::translation_memory::semantic_search_translation_memory,
            commands::translation_memory::build_tm_vector_index,
            ...
            // Memory Scanner (Universal Hook Scaffold)
            commands::memory_scanner::inject_overlay_hook,
            commands::memory_scanner::detach_overlay_hook,
            commands::memory_scanner::get_hook_status,
            commands::memory_scanner::listen_memory_strings,

            // Font Patcher
            commands::font_patcher::scan_game_fonts,
            commands::font_patcher::patch_font_with_cyrillic,

            // AI Commission (Multi-Agent)
            commands::ai_commission::commission_translate_text,

            // NER Extractor
            commands::ner_extractor::extract_glossary_terms,

            // Mod Manager
            commands::mod_manager::list_installed_mods,
            commands::mod_manager::install_community_mod,
            commands::mod_manager::remove_mod,
            commands::mod_manager::resolve_mod_conflicts,

            // Texture Translator (Inpainting)
            commands::texture_translator::translate_game_texture,

            // Voice Cloner (RVC/VITS)
            commands::voice_cloner::clone_character_voice,

            // Git Community Translations
            commands::git_community::push_translation_to_community,
            commands::git_community::pull_community_translations,

            // Interactive Timeline
            commands::timeline_manager::create_timeline_snapshot,
            commands::timeline_manager::restore_timeline_snapshot,
            commands::timeline_manager::list_timeline_snapshots,

            // In-Game Fix Hotkey
            commands::ingame_fix::register_ingame_fix_hotkey,
            commands::ingame_fix::submit_ingame_fix,

            // Dynamic Theme
            commands::dynamic_theme::extract_game_theme,

            // Pipeline Nodes
            commands::pipeline_nodes::save_custom_pipeline,
            commands::pipeline_nodes::load_custom_pipelines,
            commands::pipeline_nodes::execute_pipeline,

            // Steam Deck Integration
            commands::steamdeck_integration::add_to_steamdeck_shortcuts,
            commands::steamdeck_integration::is_running_on_steamdeck,

            // Gamification
            commands::gamification::get_user_profile,
            commands::gamification::add_exp,

            // Smart Placeholders
            commands::smart_placeholders::tokenize_placeholders,
            commands::smart_placeholders::detokenize_placeholders,
        ])
        .setup(|app| {
            // ═══════════════════════════════════════════════════
