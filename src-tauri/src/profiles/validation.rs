...
impl ProfileValidator {
    ...
    pub fn validate_profile_name(&self, name: &str) -> ProfileNameValidationResult {
        ...
        // Controlla caratteri validi (supporta tutti i caratteri Unicode: latino, cirillico, CJK, ecc.)
        let is_valid_char = |c: char| c.is_alphanumeric() || c == '_' || c == '-' || c == ' ';
        if !sanitized_name.chars().all(is_valid_char) {
