  ...
  const authenticateProfile = useCallback(async (name: string, password: string): Promise<boolean> => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const errorMsg = response.error?.includes('non trovato') || response.error?.includes('not found') ? 'Профиль не найден' :
                         response.error?.includes('password') || response.error?.includes('non valida') ? 'Неверный пароль' :
                         (response.error || 'Ошибка аутентификации');
        clientLogger.warn(`⚠️ useProfiles: Ошибка входа: ${errorMsg}`);
        setError(errorMsg);
        return false;
      }
    } catch (err: unknown) {
      clientLogger.error(`❌ useProfiles: Ошибка подключения: ${String(err)}`);
