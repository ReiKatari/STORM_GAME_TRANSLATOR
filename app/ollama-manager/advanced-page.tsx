...
export default function OllamaAdvancedPage() {
  ...
  const handleDetectGPU = async () => {
    ...
    try {
      ...
      setGpuInfo(info);
      if (info) {
        toast.success(t('ollama.gpuDetected'), { description: `${info.name} - ${(info.vramTotalMB / 1024).toFixed(1)}GB VRAM` });
      }
    } catch (error) {
                ...
                >
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <Card className="bg-slate-800/50">
                      <CardContent className="p-4">
                        ...
                        <div className="font-semibold">{(gpuInfo.vramTotalMB / 1024).toFixed(1)} GB</div>
                      </CardContent>
                    </Card>
                    <Card className="bg-slate-800/50">
                      <CardContent className="p-4">
                        <div className="text-sm text-muted-foreground">{t('ollama.availableVRAM')}</div>
                        <div className="font-semibold text-emerald-400">{(gpuInfo.vramAvailableMB / 1024).toFixed(1)} GB</div>
                      </CardContent>
                    </Card>
                    <Card className="bg-slate-800/50">
                      <CardContent className="p-4">
                        <div className="text-sm text-muted-foreground">{t('ollama.cudaSupport')}</div>
                        <Badge variant={gpuInfo.cudaSupport ? "default" : "secondary"} className={gpuInfo.cudaSupport ? "bg-emerald-500/20 text-emerald-400" : ""}>
                          {gpuInfo.cudaSupport ? t('common.yes') : t('common.no')}
                        </Badge>
                      </CardContent>
                    </Card>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>{(100 - (gpuInfo.vramAvailableMB / (gpuInfo.vramTotalMB || 1) * 100)).toFixed(1)}%</span>
                    </div>
                    <Progress 
                      value={100 - (gpuInfo.vramAvailableMB / (gpuInfo.vramTotalMB || 1) * 100)} 
                      className="h-2"
                    />
                  </div>
          ...
          <Card className="border-slate-800/50 bg-slate-900/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('ollama.selectedCount') ? `${t('ollama.selectedCount')}: ${ensembleModels.length}/3` : `Выбрано: ${ensembleModels.length}/3`}
                </p>
              </div>
