      ...
      return (
        <div className="text-center py-10">
          <p className="text-gray-400 mb-2">
            {searchTerm
              ? `${t('libraryPage.noGamesFound') || 'Игры не найдены по запросу'} "${searchTerm}".`
              : (t('libraryPage.emptyLibrary') || 'Ваша библиотека пуста.')}
          </p>
          {searchTerm && hasActiveFilters && (
            <p className="text-sm text-yellow-500 mb-2">
              ⚠️ {t('libraryPage.activeFiltersWarning') || 'Активные фильтры могут скрывать некоторые игры.'}
            </p>
          )}
          {searchTerm && safeGames.length > 0 && (
            <p className="text-sm text-gray-500">
              {safeGames.length} {t('libraryPage.gamesInLibraryHint') || 'игр в библиотеке. Попробуйте сбросить фильтры.'}
            </p>
          )}
          {!searchTerm && (
            <p className="text-sm text-gray-500">
              {t('libraryPage.emptyLibraryHint') || 'Нажмите кнопку «Обновить», чтобы добавить игры в библиотеку.'}
            </p>
          )}
        </div>
      );
                    ...
                    <div className="flex items-center gap-3 mr-4">
                      {game.is_installed && (
                        <span className="flex items-center gap-1 text-2xs font-bold text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-md border border-emerald-500/20">
                          ...
                          {t('common.installed') || 'Установлено'}
                        </span>
                      )}
                      {game.is_vr && (
        ...
        <div className="relative flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              title={t('libraryPage.rescanTooltip') || 'Сканировать все установленные игры'}
            >
              <RefreshCw className="h-4 w-4 group-hover:rotate-180 transition-transform duration-500" />
              <span className="text-[11px] font-semibold tracking-wide">{t('common.scan') || 'Сканировать'}</span>
            </button>
            <button
              ...
              title={t('libraryPage.ptRankTooltip') || 'Рейтинг P.T. — сложность перевода игр'}
            >
              <Brain className="h-4 w-4 text-purple-400 group-hover:text-purple-300 transition-colors" />
              <span className="text-[11px] font-semibold tracking-wide">{t('libraryPage.ptRank') || 'Рейтинг P.T.'}</span>
            </button>
            <button
              ...
              title={t('libraryPage.dryRunTooltip') || 'Тестовый запуск — проверка конвейера'}
            >
              <Search className="h-4 w-4 text-indigo-400 group-hover:text-indigo-300 transition-colors" />
              <span className="text-[11px] font-semibold tracking-wide">{t('libraryPage.dryRun') || 'Тест'}</span>
            </button>
            <button 
              onClick={async () => {
              ...
              title={t('common.scaricaNomiCorrettiDeiGiochiDalDatabaseRemoto') || 'Обновить базу названий игр'}
            >
              <Download className="h-4 w-4 text-slate-400 group-hover:text-indigo-300 transition-colors" />
              <span className="text-[11px] font-semibold tracking-wide">{t('libraryPage.namesDb') || 'База имен'}</span>
            </button>
          </div>
        </div>
      </div>
